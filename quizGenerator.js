...
المطلوب: تصميم **كويز احترافي مطبوع** يحتوي على ${count} سؤالاً بصيغة HTML/A4 جاهزة للطباعة كـ PDF وفق معايير البكالوريا 2027.

⚠️ **وضع الكويز الحالي: ${questionMode === 'essay' ? 'مقالي تحليلي' : 'اختيار من متعدد'}**
${questionMode === 'essay' ? '- أنشئ 5 أسئلة مقالية/تحليلية فقط، بدون أي MCQ أو صح/خطأ. لكل سؤال: المطلوب، المعطيات إن وجدت، خطوات الحل، ثم إجابة نموذجية كاملة.' : '- أنشئ أسئلة اختيار من متعدد فقط، مع أربعة اختيارات لكل سؤال.'}
...
2. **الأسئلة المقالية والتحليلية:** اذكر المعطيات والمطلوب والوحدات بوضوح، ثم أضف بعد كل سؤال مقالي أو مسألة عددية مساحة بعنوان «مساحة خطتك وحلك باليد (فكر وحل هنا أولاً):» مع 3–5 أسطر مخصصة للحل.
3. **مساحة «حل بإيدك»:** لا تستخدم workspace-area بعد MCQ أو صح/خطأ. استخدمها حصراً بعد السؤال المقالي المركب أو المسألة الفيزيائية/العددية التي تحتاج خطوات حل يدوية.
4. **مفتاح الإجابات:** ضعه داخل وسم section يحمل class باسم answer-key وanswer-key-section في نهاية الكويز، وافصل إجابة كل سؤال في بطاقة مستقلة.
...
/**
 * إزالة مفتاح الإجابة فقط مع الإبقاء على الأسئلة ومساحات الحل والتنسيق.
 */
export function createUnansweredQuizHtml(html) {
  if (typeof html !== 'string' || !html.trim()) throw new Error('محتوى كويز PDF غير صالح');
  const withoutAnswerKey = html
    .replace(/<section\b[^>]*class=["'][^"']*\banswer-key-section\b[^"']*["'][\s\S]*?<\/section>/gi, '')
    .replace(/<div\b[^>]*class=["'][^"']*\banswer-key-section\b[^"']*["'][\s\S]*?<\/div>/gi, '');
  return withoutAnswerKey
    .replace(/<\/head>/i, '<style>.answer-key, .answer-key-section { display: none !important; }</style></head>')
    .replace(/مفتاح الإجابات النموذجية والتعليلات العلمية/g, 'نسخة الطالب — الأسئلة ومساحات الحل')
    .replace(/إجابة نموذجية كاملة/g, '');
}
